...
In my banking copilot that step was an LLM call with a structured output. This
...
BANKING77 is English. An Israeli bank's customers mostly are not, and a banking
assistant sold as multilingual has to work in their language, so an English-only accuracy figure answers the easy
...
Training needs a CUDA GPU. Every number above came from a GTX 1080 (8 GB).

```bash
python -m venv .venv && . .venv/bin/activate
pip install torch --index-url https://download.pytorch.org/whl/cu126
pip install -r requirements.txt
export ANTHROPIC_API_KEY=...   # for the LLM baseline and data generation
```
...
[BANKING77](https://huggingface.co/datasets/PolyAI/banking77) (Casanueva et al.,
2020), CC BY 4.0, downloaded on first run. The Hebrew slice and the negative
examples are generated by the scripts above and are not committed.

## License

Code: MIT, see [LICENSE](LICENSE). BANKING77 keeps its own CC BY 4.0 license.
